...
from stateful_chat.llm import OllamaLLM
from stateful_chat.manager import LLMSummaryMemory
import ollama
import uuid
import json
import os
...
if "chat_session" not in st.session_state:
    ...
    # configure memory
    llm = OllamaLLM(model=cs.llm)
    st.session_state.memory = LLMSummaryMemory(llm)
    # pass defaults over to session
    st.session_state.chat_session.instruct_format = llm.instruct_format
    st.session_state.chat_session.sampling_options = llm.sampling_options
    st.session_state.chat_session.prompt_full_summary = st.session_state.memory.prompt_full_summary
    st.session_state.chat_session.prompt_msg_summary = st.session_state.memory.prompt_msg_summary
    st.session_state.chat_session.prompt_entity_list = st.session_state.memory.prompt_entity_list
...
def load_state():
    ...
    llm = OllamaLLM(model=st.session_state.chat_session.llm)
    llm.sampling_options = st.session_state.chat_session.sampling_options
    llm.instruct_format = st.session_state.chat_session.instruct_format
    st.session_state.memory = LLMSummaryMemory(llm)
    st.session_state.memory.user_role = st.session_state.chat_session.user_role
    st.session_state.memory.ai_role = st.session_state.chat_session.ai_role
    st.session_state.memory.message_summaries = st.session_state.chat_session.message_summaries
    st.session_state.memory.full_summary = st.session_state.chat_session.full_summary
    st.session_state.memory.entity_list = st.session_state.chat_session.entity_list
    st.session_state.memory.prompt_full_summary = st.session_state.chat_session.prompt_full_summary
    st.session_state.memory.prompt_entity_list = st.session_state.chat_session.prompt_entity_list
    st.session_state.memory.prompt_msg_summary = st.session_state.chat_session.prompt_msg_summary
...
with tab_main:
    ...
    st.session_state.memory.llm.model = st.session_state.chat_session.llm
    ...
    st.session_state.memory.llm.instruct_format = st.session_state.chat_session.instruct_format
    # Base system prompt
    ...
    if st.button(label="Summarize"):
        # add messages to the memory manager
        st.session_state.memory.add_messages(st.session_state.chat_session.messages[0:num_summ])
        # update top-level summary and entity list
        st.session_state.chat_session.full_summary = st.session_state.memory.full_summary
        st.session_state.chat_session.entity_list = st.session_state.memory.entity_list
        #st.session_state.chat_session.update_full_summary(0, num_summ)
        # make summary of the message chunk
        st.session_state.chat_session.message_summaries = st.session_state.memory.message_summaries
...
with tab_mem:
    ...
    if len(st.session_state.chat_session.message_summaries) == 0:
        ...
        # update memory manager too, if there are any
        st.session_state.memory.message_summaries[-1] = st.session_state.chat_session.message_summaries[-1]
...
with tab_settings:
    formatted_text = json.dumps(st.session_state.chat_session.sampling_options, indent=2)
    edited_text = st.text_area("Edit LLM sampling parameters:", formatted_text, height = 500)
    st.session_state.chat_session.sampling_options = json.loads(edited_text)
    # update memory manager too
    st.session_state.memory.llm.sampling_options = st.session_state.chat_session.sampling_options
